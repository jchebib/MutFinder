...
parser.add_argument("--start", dest="start", default=1, type=int, help="The start position in the range of desired sequences")
parser.add_argument("--finish", dest="finish", default=195000000, type=int, help="The end position in the range of desired sequences")
...
####################### perform same filters with founders exempt to capture variants in both founders AND descendents 
			if(p!=0 and p!=1):  # not founders
				if(v.gt_types[p and (0 or 1)]!=0 and np.all(np.delete(v.gt_types, [0,1,p])==0)):  # check only focal sample and either founder are not Homo REF
					if((sum(np.delete(v.gt_alt_depths, [0,1,p])) <= max_impurity)): # check non-focal samples combined have fewer ALT reads than max_impurity
